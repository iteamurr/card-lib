"""
    Implementation of tools for working with a ``Collection`` object.
"""
...
class Collection:
    ...
    def collections(self) -> None:
        ...
        navigation = Tools.navigation_creator(
            "CoLSe", len(collections_list), level=level, per_page=per_page
        )
    ...
    def new_collection(self) -> None:
        ...
        with Select("bot_messages") as select:
            self.title = select.bot_message("new_collection", self.locale)
        ...
        self.menu = CollectionTemplates.new_collection_template(
    ...
    def add_collection_session(self) -> None:
        ...
        with Select("bot_messages") as select:
            self.title = select.bot_message("create_collection", self.locale)
